...
def letter_grades(highest):
    ...
    """
    factor = (highest - 40) // 4
    return [41, 41 + factor, 41 + 2 * factor, 41 + 3 * factor]
...
def student_ranking(student_scores, student_names):
    ...
    """
    ranked_names = enumerate(student_names)
    return [f"{index + 1}. {name}: {student_scores[index]}" for index, name in ranked_names]
